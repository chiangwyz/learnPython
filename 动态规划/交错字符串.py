...
创建一个二维数组 dp，其中 dp[i][j] 表示 s1 的前 i 个字符和 s2 的前 j 个字符是否能交错组成 s3 的前 i+j 个字符。

可以按以下步骤进行：
1. 初始化 dp 数组。dp[0][0] 为 True，因为两个空字符串可以组成一个空字符串。
    接下来，我们初始化 dp 的第一行和第一列。
    dp[i][0] 为 True，如果 s1 的前 i 个字符和 s3 的前 i 个字符相同；
...
class Solution:
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        ...
        s1[i - 1] 是否等于 s3[i - 1]：检查 s1 的第 i 个字符是否与 s3 的第 i 个字符相同。
        """
        # 初始化 dp 的第一列
        ...
        s2[j - 1] 是否等于 s3[j - 1]：检查 s2 的第 j 个字符是否与 s3 的第 j 个字符相同。
        """
        # 初始化 dp 的第一行
