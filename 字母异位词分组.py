...
class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        在 defaultdict 中，当你尝试访问一个还不存在的键时，它会自动为该键创建一个默认值。
        在这个例子中，list 被传递给 defaultdict 作为默认工厂函数，意味着如果你尝试访问 defaultdict 中还不存在的键，它将自动为该键创建一个新的空列表作为默认值。
        
        简而言之，defaultdict(list) 创建了一个字典，当访问一个新键时，会自动关联一个空列表，而不是抛出一个 KeyError，这在许多场景下可以简化代码。
        例如，在需要收集元素到列表中并且不希望手动检查键是否存在的情况下，它非常有用。
        """
