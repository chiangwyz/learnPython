...
循环遍历数组 (for i in range(len(nums) - 1)):
这个循环不包括数组的最后一个元素，因为当到达或超过最后一个元素时，就不需要再跳跃了。
在每一步中，更新farest为当前位置加上该位置能跳的最大长度（i + nums[i]）和之前的farest中的较大值。
判断是否需要进行新的跳跃 (if i == current_end):

当i达到current_end时，意味着已经达到了上一次跳跃能到达的最远位置。
这时需要进行新的跳跃，因此将jumps增加1。
同时，更新current_end为farest，即新的跳跃能到达的最远位置。
返回跳跃次数 (return jumps):
