""""
可以采用一种分治方法来寻找两个已排序数组的中位数，达到时间复杂度为 O(log(min(m,n)))。

原理：中位数是将一个集合划分为两个长度相等的子集，其一个子集中的所有元素都不大于另一个子集的任何元素。
这里，我们将问题转化为寻找两个排序数组中的第 k 小的数，其中 k = (m+n)/2 (或k=((m+n)/2) + 1)取决于 m+n 是奇数还是偶数。

具体步骤：
1. 首先，定义一个辅助函数，用于寻找两个排序数组中的第 k 小的数。
...
class Solution:
    def findMedianSortedArrays(self, nums1: list[int], nums2: list[int]) -> float:
        def findKthElement(arr1: list[int], arr2: list[int], k: int):
            """
...
print(solution.findMedianSortedArrays([2, 3, 5, 6], [1, 4, 6]))      # 4
